...
$query = "SELECT DISTINCT a.id, a.qty, a.seller,a.buyer_fee, a.tax_fee, a.paid, a.feedback_win, a.bid, a.auction, a.shipped, b.title, b.ends, b.shipping_cost, b.additional_shipping_cost, b.shipping, u.nick, u.email
		FROM " . $DBPrefix . "winners a
		LEFT JOIN " . $DBPrefix . "auctions b ON (a.auction = b.id)
		LEFT JOIN " . $DBPrefix . "users u ON (u.id = a.seller)
		WHERE (b.closed = 1 OR b.bn_only = 1) AND b.suspended = 0
...
while ($row = $db->fetch())
{
	...
	$totalcost =$totalcost + $row['buyer_fee'] + $row['tax_fee'];
