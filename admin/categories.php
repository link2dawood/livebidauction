 ...
 *   (at your option) any later version. Although none of the code may be sold. If
 *   you have been sold this script, get a refund.
 ***************************************************************************/
...
if (isset($_POST['action']))
{
	...
	{
		// Update all categories that aren't being deleted
		if (isset($_POST['categories']) && is_array($_POST['categories']))
		{
			foreach ($_POST['categories'] as $k => $v)
			{
				...
				{
					// Ensure 'cat_description' is included and defaults to an empty string if not provided
					$cat_description = isset($_POST['description'][$k]) ? $_POST['description'][$k] : '';
					...
					$params[] = array(':cat_description', $cat_description, 'str');
		...
		{
			// Ensure 'cat_description' defaults to an empty string if not provided
		...
		// Add categories in bulk if provided
	...
	if ($_POST['action'] == "Yes")
	{
		...
		if (isset($_POST['delete']) && is_array($_POST['delete']))
		{
			foreach ($_POST['delete'] as $k => $v)
			{
				...
				if ($v == 'delete')
				{
					// Never delete categories directly, use this function to avoid DB issues
				...
				elseif ($v == 'move')
				{
					...
					{
						// Move parent category
						$catscontrol->move($k, $_POST['moveid'][$k]);
						// Delete the parent and raise the children up one level
