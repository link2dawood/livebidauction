...
<!-- IF PAGE eq 1 -->
	...
	<!-- ENDIF -->
			<form class="form-horizontal" id = "confirmBid" name="bid" action="{ASSLURL}bid.php" method="post">
...
<!-- ELSE -->
			...
			<div class="alert alert-success" role="alert">{L_699} {BID} {L_700}<br>
                                   {L_272}&nbsp;
			<a href="{SITEURL}item.php?id={ID}">{SITEURL}item.php?id={ID}</a>
                        </div>
<!-- ENDIF -->
		<script>
		$(document).on("submit", "#confirmBid", function(e){
					e.preventDefault();
