		...
		if(isset($_POST['action']) && $_POST['action'] == 'update')
		{
			...
			else
			{
				...
				$db->query($query, $params);
        		    ...
        		    $fee_from = isset($_POST['new_fee_from']) ? $_POST['new_fee_from'] : 0;
        		    $fee_to = isset($_POST['new_fee_to']) ? $_POST['new_fee_to'] : 0;
        		    $new_type = isset($_POST['new_type']) ? $_POST['new_type'] : 'flat';
        		    $query = "INSERT INTO " . $DBPrefix . "fees VALUES
							(NULL, :fee_from, :fee_to, :new_type, :value, :type)";
					$params = array();
					$params[] = array(':fee_from', $system->input_money($fee_from), 'float');
					$params[] = array(':fee_to', $system->input_money($fee_to), 'float');
					$params[] = array(':new_type', $new_type, 'str');
					$params[] = array(':value', $system->input_money($_POST['value']), 'float');
					$params[] = array(':type', $_GET['type'], 'str');
					$db->query($query, $params);
