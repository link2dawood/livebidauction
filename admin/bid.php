...
$query = "SELECT id FROM " . $DBPrefix . "users WHERE is_floor = 1 LIMIT 1";
...
if ($db->numrows() == 0)
{
	$template->assign_vars(array(
			'TITLE_MESSAGE' => $MSG['415'].'  <a href="listfloorauctions.php" >Back</a>',
			'BODY_MESSAGE' => $ERR_606
			));
	include 'header.php';
	$template->set_filenames(array(
			'body' => 'listfloorauctions.tpl'
			));
...
$differnrce = ($bid - $high_bid);
/*if($user->user_data['is_floor'] != 1 && $atype != 2){
if(round($differnrce,2) < round($increment,2)){
	$errmsg = $ERR_5047 .' '.$next_bid;
}
} */
...
if (isset($_POST['action']) && !isset($errmsg))
{
	...
	elseif ($atype == 2 && !isset($errmsg)) // dutch auction
	{
		...
		if ($db->numrows() > 0)
		{
			...
			// Add Increments amount to bid depending on bid amount that was checked previously

			$bid = $bid + get_increment($bid, false);
...
header("Location:listfloorauctions.php");
exit;
}
