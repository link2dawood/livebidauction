...
$query = "SELECT a.*, ac.counter, u.nick, u.reg_date, u.city, u.country, u.zip, a.going_once FROM " . $DBPrefix . "auctions a
...
$going_data = '';
if($auction_data['going_once'] == 1 && $auction_data['going_twice'] == 0)
{
	$going_data = "Going Once....";
}
elseif($auction_data['going_twice'] == 1 && $auction_data['going_once'] == 0)
{
	$going_data = "Going Twice....";
}
elseif($auction_data['going_once'] == 0 && $auction_data['going_twice'] == 0)
{
	$going_data = "Auction is live...Place your bid!";
}
$pending_payments = 0;
if ($user->logged_in)
{
...
$db->query($query, $params);
$pending_payments = $db->result('COUNT');
}
$template->assign_vars(array(
		'ID' => $auction_data['id'],
		'TITLE' => htmlspecialchars($auction_data['title']),
		'LIVESTREAM_LINK' => $auction_data['livestream_link'],
		...
		'PEND_PAYMENTS' => $pending_payments,
		'TEMP_SUSPEND' => isset($user->user_data['temp_suspended']) ? $user->user_data['temp_suspended'] : '2',
		...
		'B_MANUAL_HEADING' => $show_manual_heading,
		...
		'AUCTION_GOING' => htmlspecialchars($going_data),
