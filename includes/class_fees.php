	...
	private function callback_process($custom_id, $fee_type, $payment_amount, $currency = NULL)
	{
		switch ($fee_type)
		{
			...
			case 2: // pay for an item
				...
				$this->database->query($query, $params);
				//Enable User
					$query = "UPDATE " . $this->DBPrefix . "users SET temp_suspended = 1 WHERE id = :user_id";
					$params = array();
					$params[] = array(':user_id', $custom_id, 'int');
					$this->database->query($query, $params);
			break;
			case 3: // pay signup fee (live mode)
				$query = "UPDATE " . $this->DBPrefix . "users SET suspended = 0, reg_fee_paid = 1 WHERE id = :custom_id";
				$params = array(
					array(':custom_id', $custom_id, 'int')
				);
				$this->database->query($query, $params);
