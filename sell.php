...
$system->ctime = $system->ctime + (16*3600);
...
function resize($source_image, $finished, $tn_w, $tn_h, $quality = 100, $wmsource = false)//if you wish to add a watermark replace ' $wmsource = false '  for the watermarks location ' $wmsource = "path_to_warkmark.png" '
{
    ...
    if (imagejpeg($final, $finished, $quality)) {
	...
	case 4: // finalise auction (submit to db)
		...
		else
		{
		  ...
		  //  die();
			...
			$_SESSION['SELL_description'] = $_SESSION['SELL_description'];
...
// 			die();
			...
			if ($system->SETTINGS['picturesgallery'] == 1 && count($UPLOADED_PICTURES) > 0)
			{
            	...
            	foreach ($UPLOADED_PICTURES as $k => $v)
            	{
	                ...
	                resize($source, $destination, 1024, 768);
	...
	case 3: // confirm auction
		...
		if ($noerror)
		{
			...
			$template->assign_vars(array(
					...
					'LIVESTREAM_LINK' => nl2br($livestream_link),
...
// 		die();
		...
		$template->assign_vars(array(
				...
				'LIVESTREAM_LINK' => htmlspecialchars($livestream_link),
