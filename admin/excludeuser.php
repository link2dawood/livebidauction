...
if (isset($_POST['action']) && $_POST['action'] == "Yes")
{
	...
	$query = "SELECT name, email,temp_suspended,is_floor, suspended FROM " . $DBPrefix . "users WHERE id = :user_id";
	$params = array();
	$params[] = array(':user_id', intval($_REQUEST['id']), 'int');
	...
	if ($_POST['mode'] == 'temp_suspend')
	{
		//Check for Floor user
		...
		$was_suspended = ($USER['temp_suspended'] == 0 ? true : false);
		...
		else
		{
			...
			$message_error = "Floor user Can't be suspended!";
			$_SESSION['error_message'] = $message_error;
		}
	}
		if ($_POST['mode'] == 'active')
	{
		...
		$was_suspended = ($USER['temp_suspended'] == 1 ? true : false);
...
if(isset($_GET['temp_susp'])){
switch ($user_data['temp_suspended'])
{
	case 1:
		$action = $MSG['305a'];
		$question = $MSG['308'];
