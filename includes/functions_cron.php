...
function sortFees()
{
	...
	global $endauc_fee, $tax_id, $tax_fee, $buyer_fee, $buyer_fee_type, $bf_paid, $ff_paid, $NOW, $db;
	...
	if ($system->SETTINGS['fee_type'] == 1 && $buyer_fee > 0)
	{
		...
		if($Auction['tax'] == 1 && $Auction['taxinc'] == 1){
			if($tax_fee > 0){
				$tax_value = ($tax_fee / 100) * floatval($Auction['current_bid']);
			}
		}
		...
		$db->query($query, $params);
		$query = "UPDATE " . $DBPrefix . "winners SET buyer_fee = :buyer_fee, tax_fee = :tax_fee, tax_id = :tax_id WHERE winner = :winner_id AND auction = :auc_id";
		$params = array();
		$params[] = array(':buyer_fee', $fee_value, 'float');
		$params[] = array(':tax_fee', $tax_value, 'float');
		$params[] = array(':tax_id', $tax_id, 'int');
		$params[] = array(':winner_id', $Winner['id'], 'int');
		$params[] = array(':auc_id', $Auction['id'], 'int');
		$db->query($query, $params);
		
		$query = "INSERT INTO " . $DBPrefix . "useraccounts (user_id, auc_id, date, buyer, total, paid) VALUES
				(:winner_id, :auc_id, :time, :buyer, :buyer_fee, 1)";
		$params = array();
		$params[] = array(':buyer_fee', $fee_value, 'float');
		$params[] = array(':buyer', $fee_value, 'float');
		$params[] = array(':winner_id', $Winner['id'], 'int');
		$params[] = array(':auc_id', $Auction['id'], 'int');
