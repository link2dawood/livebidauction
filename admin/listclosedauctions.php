...
$query = "SELECT a.id, u.nick, a.title, a.starts, a.ends, a.suspended, c.cat_name, 
                 COUNT(w.id) as winners, COUNT(r.id) as times_reported, 
          ...
          LEFT JOIN " . $DBPrefix . "auction_moderation m ON (a.id = m.auction_id)
          WHERE m.reason IS NULL AND a.closed = 1 AND a.suspended = 0 
          GROUP BY a.id, u.nick, a.title, a.starts, a.ends, a.suspended, c.cat_name 
          ORDER BY u.nick 
