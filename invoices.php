...
// 	die();
	...
	$shipping_cost = $user->user_data['is_floor'] == 0 ? $shipping_data['shipping_total'] : 0;
	$template->assign_block_vars('to_pay', array(
			...
			'SHIPPING_APPLIED' => (($user->user_data['is_floor'] == 0) ? TRUE : FALSE),
			...
			'TOTAL' => $system->print_money(($row['bid'] * $row['qty']) + $shipping_cost + $row['buyer_fee'] + $row['tax_fee']),
			'AUC_ID' => $row['auc_id'],
			'WINID'=> $row['id'],
