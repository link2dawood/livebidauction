...
if (isset($_POST['action']) && $_POST['action'] == 'update')
{
	if ($_POST['name'] && $_POST['email'])
	{
		...
		else
		{
		  ...
		  $query = "SELECT nick FROM " . $DBPrefix . "users WHERE nick = :username LIMIT 1";
$params = array();
$params[] = array(':username', $system->cleanvars($_POST['username']), 'str');
$db->query($query, $params);  // Query is executed here

if ($db->numrows() > 0) {
    // Username already exists
    $template->assign_block_vars('alerts', array('TYPE' => 'error', 'MESSAGE' => $ERR_003U));
} else {
    // Code for inserting user
}


			//var_dump($_POST['user_floor']); exit;
			$query = "INSERT INTO " . $DBPrefix . "users 
    (name, nick, email, address, city, prov, country, zip, phone, is_floor, birthdate, `groups`, balance, password) 
    VALUES 
    (:name, :nick, :email, :address, :city, :prov, :country, :zip, :phone, :is_floor, :birthdate, :groups, :balance, :password)";
			...
			$params[] = array(':is_floor', intval($_POST['user_floor']), 'int');
...
$template->assign_vars(array(
		...
		'ISFLOOR' => (isset($_POST['user_floor'])) ? $_POST['user_floor'] : '',
		'REQUIRED' => array(
