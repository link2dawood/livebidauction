...
function setvars()
{
	...
	global $_POST, $_SESSION, $system, $custom_end, $a_ends, $custom_end, $caneditstartdate,$_COOKIE;
    // echo '<pre> _POST DATA <br>';
    // print_r($_POST);
    // echo '<br>';
    //  echo '<pre> _SESSION DATA <br>';
    //   print_r($_SESSION);
    // die();
	...
	$_SESSION['spdescription'] = (isset($_POST['sdescription'])) ? $_POST['sdescription'] : $_SESSION['SELL_description'];
	$sdescription = (isset($_POST['sdescription'])) ? $_POST['sdescription'] : $_SESSION['SELL_description'];
// 	print_r($sdescription);
// 	die();
	...
	$is_taxed = (isset($_POST['is_taxed'])) ? $_POST['is_taxed'] == 'y' ? 1 : 0 : 0;
	$tax_included = (isset($_POST['tax_included'])) ? $_POST['tax_included'] == 'y' ? 1 : 0 : 0;
...
// 	$_SESSION['SELL_file_uploaded'] = $imgtype;
	...
	$_SESSION['SELL_description'] = $_SESSION['spdescription'];
...
// 	$_SESSION['SELL_buy_now_only'] = $buy_now_only;
	$_SESSION['SELL_starts'] = strtotime($a_starts);
	...
	$_SESSION['SELL_is_taxed1'] = $is_taxed;
// 	print_r()
	$_SESSION['SELL_tax_included1'] = $tax_included;
	$_SESSION['SELL_caneditstartdate'] = $caneditstartdate;
	
	setcookie('nomi_SELL_is_taxed', $is_taxed, time() + (86400), "/");
	setcookie('nomi_SELL_tax_included', $tax_included, time() + (86400), "/");
...
function makesessions()
{
	...
	global $a_ends, $custom_end, $caneditstartdate ,$_COOKIE;
    // echo '<pre> _POST DATA <br>';
    // print_r($_POST);
    // echo '<br>';
    //  echo '<pre> _SESSION DATA <br>';
...
function addauction()
{
	global $DBPrefix, $_SESSION, $user, $a_starts, $a_ends, $payment_text, $system, $fee, $db,$_COOKIE;
// 	print_r($_COOKIE['nomi_SELL_is_taxed']);
// 	die();
	...
	$params[] = array(':catone', $_SESSION['SELL_sellcat1'], 'int');
	$params[] = array(':cattwo', $_SESSION['SELL_sellcat2'], 'int');
	$params[] = array(':min_bid', ($_SESSION['SELL_buy_now_only'] == 0) ? $_SESSION['SELL_minimum_bid'] : $_SESSION['SELL_buy_now_price'], 'float');
	$params[] = array(':shipping_cost', $_SESSION['SELL_shipping_cost'], 'float');
	$params[] = array(':additional_shipping_cost', $_SESSION['SELL_additional_shipping_cost'], 'float');
	$params[] = array(':reserve_price', ($_SESSION['SELL_with_reserve'] == 'yes') ? $_SESSION['SELL_reserve_price'] : 0, 'float');
	$params[] = array(':buy_now', ($_SESSION['SELL_with_buy_now'] == 'yes') ? $_SESSION['SELL_buy_now_price'] : 0, 'float');
	$params[] = array(':auction_type', $_SESSION['SELL_atype'], 'int');
	$params[] = array(':duration', $_SESSION['SELL_duration'], 'int');
	$params[] = array(':increment', $_SESSION['SELL_customincrement'], 'float');
	$params[] = array(':shipping', $_SESSION['SELL_shipping'], 'int');
	$params[] = array(':payment', $payment_text, 'str');
	$params[] = array(':international', $_SESSION['SELL_international'], 'bool');
	$params[] = array(':ends', $a_ends, 'int');
	...
	$params[] = array(':featured', $_SESSION['SELL_is_featured'], 'bool');
	$params[] = array(':fee', $fee, 'float');
	$params[] = array(':tax',$_COOKIE['nomi_SELL_is_taxed'], 'bool');
	$params[] = array(':taxinc',$_COOKIE['nomi_SELL_tax_included'], 'bool');
// 	taxinc
// 	echo '<pre>';
// 	print_r($_SESSION);
// 	die();
// 	$params[] = array(':taxinc', $_SESSION['SELL_tax_included1'], 'bool');
